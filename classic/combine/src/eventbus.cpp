...
void EventBus::subscribe(std::string event, std::string name) {
  std::cout << std::string("Event: ") + event + std::string(" payload=") + name << std::endl;
}
void EventBus::set_audio(std::string event, std::string cat, int prio) {
  audio_cat[event] = cat; audio_prio[event] = prio;
}
void EventBus::emit(std::string event, std::string payload) {
